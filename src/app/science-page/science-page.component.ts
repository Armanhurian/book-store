...
@Component({
  ...
  styleUrls: ['./science-page.component.css']
})
export class SciencePageComponent implements OnInit{
 ...
 inputSearchValue = new FormGroup({
  ...
  ngOnInit(): void {
    ...
    this.products = this.productService.scienceProductsInMainPage
  ...
  offerLableClick(){
    ...
    this.offerFilteredProducts =  this.productService.scienceProductsInMainPage.filter(item => item.Discount !== 0)
    ...
    if(!this.checkBoxOfferLable.nativeElement.className.includes('checkBoxActive')){
      ...
      if(this.inputElemMinPrice.nativeElement.value != 0){
        ...
        this.products = this.productService.scienceProductsInMainPage.filter(item => item.price >= Number(this.inputElemMinPrice.nativeElement.value))
  ...
  changeInputRange(event:any){
    ...
    this.rangePriceFilteredProducts = this.productService.scienceProductsInMainPage.filter( item => item.price >= Number(this.inputElemMinPrice.nativeElement.value))
  ...
  sendSpecialClick(){
    ...
    this.sendSpecialFilteredProducts = this.productService.scienceProductsInMainPage.filter(item => item.fastSending === true)
